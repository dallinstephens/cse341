...
const deleteContact = async (req, res) => {
  ...
  try {
    ...
    return res.status(404).json({
      message: `The contact with id ${contactId} failed to delete because that id was not found.`,
    });
  } catch (error) {
    return res
      .status(500)
      .json({ message: error.message || 'There was an internal server error.' });
  }
};
...
const updateContact = async (req, res) => {
  ...
  try {
    ...
    if (result.modifiedCount > 0) {
      ...
      return res
        .status(204)
        .json({ message: `The contact with id ${contactId} was successfully updated.` });
    }

    // A status of 404 means something is not found.
    return res.status(404).json({
      message: `The contact with id ${contactId} failed to update because that id was not found or there was no change in the body for that id.`,
    });
  } catch (error) {
    return res
      .status(500)
